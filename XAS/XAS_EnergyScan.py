...
from APSXASCalibration import findEnergyShiftHERFDAPS
...
EnergyShift = findEnergyShiftHERFDAPS(xasProData.XASOff_Norm, xasProData.UniXEnergy, True)
...
xasProData_one.changeValue(UniXEnergy = uniXEnergy[np.logical_and(uniXEnergy >= 7100, uniXEnergy <= 7130)])
...
if Boot:
    ...
    for ii in range(numBoot):
        ...
        xasProData_boot.changeValue(UniXEnergy = uniXEnergy[np.logical_and(uniXEnergy >= 7100, uniXEnergy <= 7130)])
...
Fit,Params,ParamsA,ParamsB,ParamsC,covA,covB,covc = \
        fitXASPiecewiseGauss(np.delete(xasProData_one.EnergyPlot,-4), np.delete(XASDiffBootF,-4), np.delete(XASOffBootF,-4), np.delete(XASOnBootF,-4), True)
...
xA = np.linspace(7109.5, 7115, 1000)
xB = np.linspace(7114, 7117, 1000)
xC = np.linspace(7117, 7121, 1000)
ax = plt.subplot2grid((10,1), (3,0), colspan = 1, rowspan = 7)
...
plt.plot(xC, lorwslope(xC,ParamsC[0],ParamsC[1],ParamsC[2],ParamsC[3],ParamsC[4]), label = 'C peak fit, ' + str(round(ParamsC[1],1)), linewidth = 5, color = pluscolor)
plt.xlabel('x-ray energy (eV)')
plt.ylabel('$(I_{on}-I_{off})/I_{off}$')
plt.ylim([-300,200])
plt.legend()
