...
def fitXASPiecewiseGauss(XEnergy, XASDiff, XASOff, XASOn, ploton):
    ...
    paramsa,cova = curve_fit(gauswslope, XEnergy[Achoice], XASDiff[Achoice], p0 = [sigAS,x0ApS,AS,offAS,slopeS])
    
    FitA = gauswslope(XEnergy, paramsa[0], paramsa[1], paramsa[2], paramsa[3], paramsa[4])
    print('Adone')

    paramsb,covb = curve_fit(gauswslope, XEnergy[Bchoice], XASDiff[Bchoice], p0 = [sigBpS,x0BpS,BS,offBS,slopeS])
