...
def makeXAS(xasRawData, xasProData, DorH, ploton):
    ...
    for jj in range(NumEnergySteps):
        ...
        DiodeFilterOff, Offset = makeOneDiodeFilter(xasRawData, selectedRuns, ploton)
        DiodeFilterOn, Offset = makeOneDiodeFilter(xasRawData, selectedRuns, ploton)
...
def makeBootXAS(xasRawData, xasProData, DorH, TF, ploton):
    ...
    AllFilter, TTFilter = makeOneFilter(xasRawData, ploton)
    AllFilter = np.logical_and(AllFilter, TF)
    print(np.shape(AllFilter))
