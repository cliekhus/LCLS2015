...
def findEnergyShift(XASOff, UniXEnergy, DorH, savecal, ploton):
    ...
    pos, pro = scisig.find_peaks(Fitted.flatten(), threshold = 0)
    
    FittedData = XASOff_Norm - polyData(UniXEnergy)
    FittedData[np.logical_or(UniXEnergy > 7118, UniXEnergy < 7111.5)] = 0
    posData, pro = scisig.find_peaks(FittedData, threshold = 0)
...
def findEnergyShiftHERFDAPS(XASOff, UniXEnergy, ploton):
    ...
    kalpha_index = np.argmax(np.sum(FeRuRIXS, axis=0))
    APSXASNorm = FeRuRIXS[:,kalpha_index]/np.sum(FeRuRIXS)*10000
    fit = np.polyfit(incident_axis, APSXASNorm, 3)
    poly = np.poly1d(fit)
    ...
    FittedData = XASOff_Norm - polyData(UniXEnergy)
    posData, pro = scisig.find_peaks(FittedData, height = 0.02)
    
    if ploton:
        plt.figure()
        plt.plot(incident_axis, Fitted, marker='.', label='APS')
        plt.plot(UniXEnergy, FittedData, marker = '.', label='LCLS')
    ...
    EnergyShift = (APSEnergies[-1]-LCLSEnergies[-1]+APSEnergies[-2]-LCLSEnergies[-2])/2
    ...
    if ploton:
        ...
        plt.plot(incident_axis, Fitted, label='APS')
        plt.plot(incident_axis[pos], Fitted[pos], 'o', label='APS fit')
        plt.plot(UniXEnergy+EnergyShift, FittedData, label='LCLS')
        plt.plot(UniXEnergy[posData]+EnergyShift, FittedData[posData], 'o', label='LCLS fit')
        plt.legend()
        plt.xlabel('X-ray energy (keV)')
        plt.title(str(EnergyShift) + ' energy shift')
