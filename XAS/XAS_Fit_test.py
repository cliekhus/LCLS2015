...
bounds = ((sigAS-.5,(AS-.005)*79006,x0AdS-2, sigBpS-.3,(BpS-.0015)*79006,x0BdS-.09, sigCpS-1,(CpS-.02)*79006,x0CdS-1, -100*79006,-1*79006),
          (sigAS+.4,(AS+.002)*79006,x0AdS+2, sigBpS+1,(BpS+.005)*79006,x0BdS+.5, sigCpS+10,(CpS+.007)*79006,x0CdS+1, 100*79006,1*79006))
#plt.plot(energy, diffxas(energy, sigAS,AS,x0AdS, sigBpS,BpS,x0BdS, sigCpS,CpS,x0CdS,3.555 ,-5e-4), label = 'start')
params_FeRu, cov_FeRu = curve_fit(diffxas, energy, diff, p0 = [sigAS,AS*79006,x0AdS, sigBpS,BpS*79006,x0BdS, sigCpS,CpS*79006,x0CdS, 3.555*79006,-5e-4*79006], bounds = bounds)
...
incident_axis = np.linspace(7105, 7125, 10000)
...
plt.plot(incident_axis, xason(incident_axis, params_FeRu[0], params_FeRu[1], params_FeRu[2], params_FeRu[3], params_FeRu[4], params_FeRu[5], 
                                params_FeRu[6], params_FeRu[7], params_FeRu[8], 0,0,0,0), label = 'extracted excited state')
plt.xlim([7110,7122])
