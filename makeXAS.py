...
def makeXAS(NumEnergySteps, NumTTSteps, Ipm2Sum, Diode2, UniXEnergy, XEnergy, Filter, LOn, XOn, TTDelay, TTSteps, ploton):
    ...
    for jj in range(NumEnergySteps):
        ...
        #SelectedRuns = list(a and b and c and d and e for a,b,c,d,e in zip(XOn, (XEnergy > UniXEnergy[jj]), (XEnergy <= UniXEnergy[jj+1]), NanCheck, Filter))
    ...
    #for jj in range(NumEnergySteps):
    #    EnergyPlot = EnergyPlot + [(UniXEnergy[jj]+UniXEnergy[jj+1])/2]
    ...
    return XASOn_Norm, XASOff_Norm, EnergyPlot