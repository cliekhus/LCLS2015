...
plt.figure()
line0 = plt.plot(Freq, np.angle(FTp), color = pluscolor)
line1 = plt.plot(Freq, np.angle(FTm), color = minuscolor)
plt.ylabel('phase')
plt.xlabel('cm$^{-1}$')
plt.legend((line0[0], line1[0]), (Energyplus +' eV', Energyminus +' eV'))
plt.title('smoothed')
...
makeSpectralPlot(Residualm, TCP, noverlap, nperseg, False)
makeSpectralPlot(Residualp, TCP, noverlap, nperseg, False)
